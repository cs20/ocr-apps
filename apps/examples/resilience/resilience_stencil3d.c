...
//Configurable params
#define N       4
#define XDIM    N       //Number of tiles or blocks on X-dimension
#define YDIM    N       //Number of tiles or blocks on Y-dimension
#define ZDIM    N       //Number of tiles or blocks on Z-dimension
...
ocrGuid_t resilientFunc(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    PRINTF("[Node %lu]: Hello from resilient EDT (%lu, %lu, %lu, %lu)\n", ocrGetLocation(), iter, x, y, z);
        ...
        PRINTF("[Node %lu]: Hello from cleanup EDT (%lu, %lu, %lu)\n", ocrGetLocation(), x, y, z);
    }
    ...
    if (iter > 3) {
        ...
        if (ocrGuidTableRemove(USER_KEY((iter - 2), x, y, z), &pEvt) == 0) {
