...
#define ITER_INDEX(x, y) ((y * XDIM) + x)

#define NUM_GHOSTS 4
#define NUM_POINTS (NUM_GHOSTS + 1)
#define EVENT_SPACE_PER_ITER (ITER_SPACE * NUM_POINTS) //Iteration space including ghost points

#define USER_KEY(i, x, y)       ((i * EVENT_SPACE_PER_ITER) + ITER_INDEX(x, y))

#define USER_KEY_SELF(i, x, y)  ((i * EVENT_SPACE_PER_ITER) + ( ITER_INDEX(x, y) * NUM_POINTS) + 0)
#define USER_KEY_LEFT(i, x, y)  ((i * EVENT_SPACE_PER_ITER) + ( ITER_INDEX(x, y) * NUM_POINTS) + 1)
#define USER_KEY_RIGHT(i, x, y) ((i * EVENT_SPACE_PER_ITER) + ( ITER_INDEX(x, y) * NUM_POINTS) + 2)
#define USER_KEY_UP(i, x, y)    ((i * EVENT_SPACE_PER_ITER) + ( ITER_INDEX(x, y) * NUM_POINTS) + 3)
#define USER_KEY_DOWN(i, x, y)  ((i * EVENT_SPACE_PER_ITER) + ( ITER_INDEX(x, y) * NUM_POINTS) + 4)

#define GUID_PARAM (sizeof(ocrGuid_t)/sizeof(u64))
#define NUM_PARAMS (1/*iter value*/ + NUM_DIMS/*coordinate value*/ + (GUID_PARAM * 6)/*shutdown guid + cleanup DBs (5 dep DBs)*/)
...
ocrGuid_t resilientFunc(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    if (iter > 2) {
        ocrGuid_t pEvt;
        if (ocrGuidTableRemove(USER_KEY_SELF((iter - 1), x, y), &pEvt) == 0)
            ocrEventDestroy(pEvt);

        if( x > 0 )
        {
             if (ocrGuidTableRemove(USER_KEY_LEFT((iter - 1), x, y), &pEvt) == 0)
                 ocrEventDestroy(pEvt);
        }
        if( x < XDIM-1 ) {
            if (ocrGuidTableRemove(USER_KEY_RIGHT((iter - 1), x, y), &pEvt) == 0)
                ocrEventDestroy(pEvt);
        }
        if( y > 0 )
        {
             if (ocrGuidTableRemove(USER_KEY_UP((iter - 1), x, y), &pEvt) == 0)
                 ocrEventDestroy(pEvt);
        }
        if( y < YDIM-1 ) {
            if (ocrGuidTableRemove(USER_KEY_DOWN((iter - 1), x, y), &pEvt) == 0)
                ocrEventDestroy(pEvt);
        }
    }*/
    ...
    if (iter < NUM_ITERS) {
        ...
        void *ptr_up = NULL;
        void *ptr_down = NULL;
        ocrDbCreate(&db, (void**)&ptr, sizeof(double)*LOCAL_MESH_SIZE*LOCAL_MESH_SIZE, DB_PROP_RESILIENT, NULL_HINT, NO_ALLOC);
        ocrDbCreate(&db_left_gp, (void**)&ptr_left, sizeof(double)*LOCAL_MESH_SIZE, DB_PROP_RESILIENT, NULL_HINT, NO_ALLOC);
        ocrDbCreate(&db_right_gp, (void**)&ptr_right, sizeof(double)*LOCAL_MESH_SIZE, DB_PROP_RESILIENT, NULL_HINT, NO_ALLOC);
        ocrDbCreate(&db_up_gp, (void**)&ptr_up, sizeof(double) *LOCAL_MESH_SIZE, DB_PROP_RESILIENT, NULL_HINT, NO_ALLOC);
        ocrDbCreate(&db_down_gp, (void**)&ptr_down, sizeof(double)*LOCAL_MESH_SIZE, DB_PROP_RESILIENT, NULL_HINT, NO_ALLOC);
        ...
        double* dptr_downout = (double *)ptr_down;
        double gp_from_left, gp_from_right, gp_from_up, gp_from_down;
    ...
    ocrGuid_t oEvt;
    ocrGuidTableGet(USER_KEY_SELF((iter + 1), x, y), &oEvt);
    ocrEventSatisfy(oEvt, db);

    if ( x > 0 ) 
    {
        ocrGuidTableGet(USER_KEY_LEFT((iter + 1), x,y), &oEvt);
        ocrEventSatisfy(oEvt, db_left_gp);
    }

    if ( x < XDIM-1 ) 
    {
        ocrGuidTableGet(USER_KEY_RIGHT((iter + 1), x,y), &oEvt);
        ocrEventSatisfy(oEvt, db_right_gp);
    }

    if ( y > 0 ) 
    {
        ocrGuidTableGet(USER_KEY_UP((iter + 1), x,y), &oEvt);
        ocrEventSatisfy(oEvt, db_up_gp);
    }

    if ( y < YDIM-1 ) 
    {
        ocrGuidTableGet(USER_KEY_DOWN((iter + 1), x,y), &oEvt);
    ...
    //Dependence input from left (x-1, y), i.e. right ghost of left cell
    if (x > 0) {
        ocrGuidTableGet(USER_KEY_RIGHT((iter + 1), (x - 1), y), &deps[1]);
    } else {
        deps[1] = NULL_GUID;
    }

    //Dependence input from right (x+1, y), i.e. left ghost of right cell
    if (x < (XDIM - 1)) {
        ocrGuidTableGet(USER_KEY_LEFT((iter + 1), (x + 1), y), &deps[2]);
    } else {
        deps[2] = NULL_GUID;
    }

    //Dependence input from up (x, y-1), i.e. down ghost of up cell
    if (y > 0) {
        ocrGuidTableGet(USER_KEY_DOWN((iter + 1), x, (y - 1)), &deps[3]);
    } else {
        deps[3] = NULL_GUID;
    }

    //Dependence input from down (x, y+1), i.e. up ghost of down cell
    if (y < (YDIM - 1)) {
        ocrGuidTableGet(USER_KEY_UP((iter + 1), x, (y + 1)), &deps[4]);
    } else {
        deps[4] = NULL_GUID;
    }
...
ocrGuid_t mainEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    //       PRINTF("[Node %lu]: DB created %lu from EDT (%lu, %lu)\n", ocrGetLocation(), db.guid, 0, i);
            ...
            for( k = 0; k < LOCAL_MESH_SIZE*LOCAL_MESH_SIZE; k++ )
            {
                dptr[k] = 1.0;
            }
            dptr = (double *)ptr_left;
            for( k = 0; k < LOCAL_MESH_SIZE; k++ )
            {
                dptr[k] = 1.0;
            }
            ...
            dbArray_left_gp[ITER_INDEX(i, j)] = db_left_gp;
            dbArray_right_gp[ITER_INDEX(i, j)] = db_right_gp;
            dbArray_up_gp[ITER_INDEX(i, j)] = db_up_gp;
            dbArray_down_gp[ITER_INDEX(i, j)] = db_down_gp;
            ...
            if (NUM_ITERS > 1) {
                ...
                ocrEventCreate(&evt1, OCR_EVENT_STICKY_T, EVT_PROP_TAKES_ARG | EVT_PROP_RESILIENT);
                u64 key = USER_KEY_SELF(2, i, j);
                ocrGuidTablePut(key, evt1);

                if ( i > 0  )
                {
                    ocrEventCreate(&evt2, OCR_EVENT_STICKY_T, EVT_PROP_TAKES_ARG | EVT_PROP_RESILIENT);
                    key = USER_KEY_LEFT(2, i, j);
                    ocrGuidTablePut(key, evt2);
                }
                if ( i < XDIM-1 ) 
                {
                    ocrEventCreate(&evt3, OCR_EVENT_STICKY_T, EVT_PROP_TAKES_ARG | EVT_PROP_RESILIENT);
                    key = USER_KEY_RIGHT(2, i, j);
                    ocrGuidTablePut(key, evt3);
                }
                if ( j > 0  )
                {
                    ocrEventCreate(&evt4, OCR_EVENT_STICKY_T, EVT_PROP_TAKES_ARG | EVT_PROP_RESILIENT);
                    key = USER_KEY_UP(2, i, j);
                    ocrGuidTablePut(key, evt4);
                }
                if ( j < YDIM-1 )
                {
                    ocrEventCreate(&evt5, OCR_EVENT_STICKY_T, EVT_PROP_TAKES_ARG | EVT_PROP_RESILIENT);
                    key = USER_KEY_DOWN(2, i, j);
    ...
    for (i = 0; i < XDIM; i++) {
        for (j = 0; j < YDIM; j++) {
            ...
            ocrGuid_t deps[NUM_DEPS];
            deps[0] = dbArray[i];
            deps[1] = (i > 0) ? dbArray_right_gp[ITER_INDEX((i-1), j)] : NULL_GUID; // I am left neighbor's right.
            deps[2] = (i < (XDIM - 1)) ? dbArray_left_gp[ITER_INDEX((i+1), j)] : NULL_GUID; // I am right neighbor's left.
            deps[3] = (j > 0) ? dbArray_down_gp[ITER_INDEX(i, (j-1))] : NULL_GUID; // I am up neighbor's down.
            deps[4] = (j < (YDIM - 1)) ? dbArray_up_gp[ITER_INDEX(i, (j+1))] : NULL_GUID; // I am down neighbor's up.
